...
const (
	key_username tcpxKey = "tcpx-username" // key-用户名
	key_account  tcpxKey = "tcpx-account"  // key-账号
	key_encrypt  tcpxKey = "tcpx-encrypt"  // key-加密秘钥
	key_reqIndex tcpxKey = "tcpx-reqIndex" // key-接口请求编号
)
...
func copyContext(ctx Context) *Context {
	...
	return &Context{
		...
		PerRequestContext:    ctx.PerRequestContext,
		Packx:                ctx.Packx,
		Stream:               ctx.Stream,
		offset:               ctx.offset,
...
// otherwise you should design your own client pool(gitlab.musadisca-games.com/wangxw/musae/framework/tcpx/clientPool/client-pool.go), and manage it
// yourself, like:
// ```
...
func (ctx *Context) Username() (string, bool) {
	usernameI, ok := ctx.GetCtxPerConn(key_username)
	if !ok {
		return "", ok
	}
...
func (ctx *Context) SetUsername(username string) {
	ctx.setCtxPerConn(key_username, username)
}

func (ctx *Context) AccountId() (string, bool) {
	accountId, ok := ctx.GetCtxPerConn(key_account)
	if !ok {
		return "", ok
	}
	return accountId.(string), ok
}

// When you want to tag an username to the context, use it, or it will be regarded as an anonymous user
func (ctx *Context) SetAccountId(accountId string) {
	ctx.setCtxPerConn(key_account, accountId)
}

func (ctx *Context) GetAccountId() string {
	accountId, ok := ctx.GetCtxPerConn(key_account)
	if !ok {
		return ""
	}
...
func (ctx *Context) EncryptKey() (string, bool) {
	symKey, ok := ctx.GetCtxPerConn(key_encrypt)
	if !ok {
		return "", ok
	}
	return symKey.(string), ok
}

func (ctx *Context) SetEncryptKey(symKey string) {
	ctx.setCtxPerConn(key_encrypt, symKey)
}

func (ctx *Context) GetEncryptKey() string {
	if baseconf.GetBaseConf().UseEncrypt == 0 {
		// 未开启加密
		return ""
	}
	...
	return symKey.(string)
}

func (ctx *Context) ReqIndex() (int32, bool) {
	idx, ok := ctx.GetCtxPerConn(key_reqIndex)
	if !ok {
		return 0, ok
	}
	return idx.(int32), ok
...
func (ctx *Context) ReplyWithMarshaller(marshaller Marshaller, messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReplyWithMarshaller(marshaller, messageID, src, keys...)
}
...
func (ctx *Context) JSON(messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReply("json", messageID, src, keys...)
}
...
func (ctx *Context) XML(messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReply("xml", messageID, src, keys...)
}
...
func (ctx *Context) TOML(messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReply("toml", messageID, src, keys...)
}
...
func (ctx *Context) YAML(messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReply("yaml", messageID, src, keys...)
}
...
func (ctx *Context) ProtoBuf(messageID int32, src interface{}, keys ...string) error {
	return ctx.commonReply("protobuf", messageID, src, keys...)
}

func (ctx *Context) commonReply(marshalName string, messageID int32, src interface{}, keys ...string) error {
	var buf []byte
	var e error
	...
	if ctx.Packx.Marshaller.MarshalName() != marshalName {
		...
		buf, e = NewPackx(marshaller).Pack(messageID, 0, src, keys...)
		if e != nil {
			return errorx.Wrap(e)
		}
		e = ctx.replyBuf(buf)
	...
	buf, e = ctx.Packx.Pack(messageID, 0, src, keys...)
	if e != nil {
		return errorx.Wrap(e)
	}
	return ctx.replyBuf(buf)
...
func (ctx *Context) commonReplyWithMarshaller(marshaller Marshaller, messageID int32, src interface{}, keys ...string) error {
	var buf []byte
	var e error
	buf, e = NewPackx(marshaller).Pack(messageID, 0, src, keys...)
	if e != nil {
...
func (ctx *Context) SendToUsername(username string, messageID int32, src interface{}) error {
	if ctx.poolRef == nil {
	...
	return ctx.SendToConn(anotherCtx, messageID, src)
}
