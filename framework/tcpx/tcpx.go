...
func (tcpx *TcpX) HeartBeatMode(on bool, duration time.Duration) *TcpX {
	...
	if on {
		tcpx.AddHandler(DEFAULT_HEARTBEAT_MESSAGEID, func(c *Context) {
			logger.Debug(fmt.Sprintf("recv '%s' heartbeat:", c.ClientIP()), c.Stream)
			c.RecvHeartBeat()
		})
	}
	return tcpx
...
func (tcpx *TcpX) HeartBeatModeDetail(on bool, duration time.Duration, throughMiddleware bool, messageID int32) *TcpX {
	...
	if on {
		tcpx.AddHandler(messageID, func(c *Context) {
			logger.Warn(fmt.Sprintf("recv '%s' heartbeat:", c.ClientIP()), c.Stream)
			c.RecvHeartBeat()
		})
	}
	return tcpx
...
func (tcpx *TcpX) ListenAndServeRaw(network, addr string) error {
	defer func() {
		if e := recover(); e != nil {
			logger.Fatalf("ListenAndServeRaw recover from panic %v", e)
			return
		}
	}()
	...
	for {
		...
		go func(ctx *Context, tcpx *TcpX) {
			defer func() {
				if e := recover(); e != nil {
					logger.Warn(fmt.Sprintf("recover from panic %v", e))
...
func (tcpx *TcpX) ListenAndServeTCP(network, addr string) error {
	defer func() {
		if e := recover(); e != nil {
			logger.Fatalf("ListenAndServeTCP recover from panic %v", e)
			//logger.Warn(string(debug.Stack()))
			return
		}
	}()
	...
	for {
		...
		go func(ctx *Context, tcpx *TcpX) {
			defer func() {
				if e := recover(); e != nil {
					logger.Errorf("recover from panic %v", e)
				}
			}()
			//defer ctx.Conn.Close()
			defer ctx.CloseConn()
...
func handleMiddleware(ctx *Context, tcpx *TcpX) {
	...
	logger.Warn(errorx.NewFromStringf("unexpected router-type %s", ctx.RouterType()))
	return
}
...
func handleMessageIDHandlers(ctx *Context, tcpx *TcpX) {
	...
	if !ok {
		logger.Warn(fmt.Sprintf("messageID %d handler not found", messageID))
		return
	}
...
func handleURLPatternHandlers(ctx *Context, tcpx *TcpX) {
	...
	if !ok {
		logger.Warn(fmt.Sprintf("urlPattern %s handler not found", urlPattern))
		return
	}
	//if messageID == tcpx.HeartBeatMessageID && !tcpx.ThroughMiddleware {
	//	handler(ctx)
...
func authWatch(ctx *Context, tcpx *TcpX) {
	if tcpx.auth {
		...
		case <-time.After(tcpx.authDeadline):
			logger.Warn("connection auth time out, closed")
			ctx.CloseConn()
			return
		case v := <-ctx.AuthChan():
			if v >= 0 {
				...
				// auth fail
				logger.Warn("connection auth fail, closed")
				ctx.CloseConn()
				return
			}
		case <-tcpx.closeAllSignal:
...
func (tcpx *TcpX) BeforeExit(f ...func()) {
	go func() {
		defer func() {
			if e := recover(); e != nil {
				logger.Debug(fmt.Sprintf("recover from %v", e))
			}
		}()
		ch := make(chan os.Signal)
		signal.Notify(ch, syscall.SIGINT, syscall.SIGTERM, syscall.SIGKILL, syscall.SIGQUIT)
		logger.Debug("receive signal:", <-ch)
		logger.Debug("prepare to stop server")
...
func (tcpx *TcpX) Start() error {
	...
	for _, v := range tcpx.properties {
		go func() {
			defer func() {
				if e := recover(); e != nil {
					logger.Warn(fmt.Sprintf("recover from '%v' \n %s", e, debug.Stack()))
				}
			}()
			logger.Debug(fmt.Sprintf("graceful restart %s server on %s", v.Network, v.Port))
