...
func (p *ProtoMsg) Str() string {
	return fmt.Sprintf("ProtoMsg:{AppId:%v,MsgId:%v,Topic:%v,UserId:%v,RoleId:%v,UAID:%v,Data:%v,ErrCode:%v,GUID:%v,Uids:%+v}",
		p.AppId, p.MsgId, p.Topic, p.UserId, p.RoleId, p.UAID, len(p.Data), p.ErrCode, p.GUID, p.Uids)
}
