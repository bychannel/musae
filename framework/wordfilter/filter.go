...
func (filter *Filter) LoadWordDict(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return 0, err
	}
...
func (filter *Filter) LoadNetWordDict(url string) (int, error) {
	c := http.Client{
...
func (filter *Filter) Load(rd io.Reader) (int, error) {
